...
class Index extends Component {
  constructor(props) {
    ...
    setCookie('userAuth', '');
    this.FireAuthUser = new FireAuthUser("http://192.168.0.15:7410", "test_module", config );
    this.userAuth = getCookie('userAuth') ? JSON.parse(getCookie('userAuth')) : false;
  ...
  async cancelAccount(){
    ...
    const res = this.userAuth.typ_login === 1 ? await this.FireAuthUser.cancelAccount(bodyData) : await this.FireAuthUser.cancelAccountAuth(bodyData)
    ...
    if(this.userAuth){
      return(
        <VerticalBox>
          <h1>{this.userAuth.user.user_name}님 로그인 환영</h1>
             <VerticalBox>
              {this.userAuth.typ_login === 1 ?<HorizonBox>
                <Label>비밀번호</Label>
                <Input type = "password" 
                       value = {this.state.pw}
                       onChange = {(e) => this.setState({pw : e.target.value})}/>
              </HorizonBox> : null}
              {this.userAuth.typ_login === 1 ? <Bt1 onClick = {() => this.verifyUser(this.userAuth.user.id, this.state.pw)}>
                이메일 인증
              </Bt1> : null}
      ...
      return(
        <VerticalBox>
          ...
          <Bt2 onClick = {() => this.googleLogin()}>
            구글 로그인
          </Bt2>
          <Bt3 onClick = {() => this.fbLogin()}>
            페이스북 로그인
          </Bt3>
        </VerticalBox>
      )
  ...
  renderJoinBox(){
    return(
      <VerticalBox>
        <HorizonBox>
          <JBt1 active  = {this.state.typ_login === 1}
                onClick = {() => this.setState({typ_login : 1, isValidId : false})}>
            이메일로 가입
          </JBt1>

          <JBt2 active  = {this.state.typ_login === 2}
                onClick = {() => this.setState({typ_login : 2})}>
            구글로 가입
          </JBt2>

          <JBt3 active  = {this.state.typ_login === 3}
                onClick = {() => this.setState({typ_login : 3})}>
            페이스북으로가입
          </JBt3>
        </HorizonBox>

        {this.state.typ_login === 1 ? <HorizonBox2>
          <Label>아이디</Label>
          <Input type = "text" 
  ...
  renderCancelBox(){
    if(this.state.isAuth){
      return(
        <VerticalBox>
          <h3>현재 아이디 : {this.userAuth ? this.userAuth.user.id : '로그인 필요'}</h3>
          {this.userAuth ? <SubmitBt onClick = {() => this.cancelAccount()}>
              탈퇴하기
            </SubmitBt>
          : null}
      ...
      return(
        <VerticalBox>
          <h3>현재 아이디 : {this.userAuth ? this.userAuth.user.id : '로그인 필요'}</h3>
          {this.userAuth.typ_login === 1 ? <HorizonBox2>
            <Label>비밀번호 </Label>
            <Input type = "password" 
          ...
          {this.userAuth ?<SubmitBt onClick = {() => this.reAuth(this.userAuth.user.id, this.state.cancelPw, this.userAuth.typ_login)}>
            인증하기
          </SubmitBt>
          : null}
        </VerticalBox>
