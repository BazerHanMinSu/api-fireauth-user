  ...
  async googleAuth(key, id) {
    ...
    if(key === "login"){
      ...
      if(!res.hasOwnProperty('results')){
        ...
        return ({status : 1500, results: '아이디와 비밀번호를 다시 확인해주세요.'})
      }
      else{
        const body = {
          id: user.email,
    ...
    // 계정인증
      ...
      return new Promise((resolve, reject) => {
        ...
        if(firebaseRes.user.W.currentUser.email === id){
          ...
          _Fetch(this.CHECK_EMAIL, '?id=' + firebaseRes.user.W.currentUser.email)
          // 자동으로 등록된 잘못입력한 이메일이 기존에 없던 이메일이라면 삭제
          .then(() => {
            firebaseRes.user.W.currentUser.delete()
            .then(function() {})
            .catch(function(error) {
              return;
            });
            resolve({status : 1500, results: '아이디와 비밀번호를 다시 확인해주세요.'});
          })
          .catch((e) => {resolve(e)})
